...
function TablaProductos({ handleFocuseado, conf, productos,focuseado,setFocuseado,pos,draw }) {
    ...
    useEffect(()=>{get_descripciones(productos,conf).then(productosDesc=>{
        setProds(productosDesc.map(function ([prod,valor],i){
            return <ProductoBusqueda handleFocuseado={handleFocuseado} draw={draw} prod={prod} valor={valor} key={i} pos={pos} conf={conf} producto={productos[i]} index={i} setFocuseado={setFocuseado} focused={focused == i ? "focuseado" : ""} />
        }))
    })},[productos,focused])
