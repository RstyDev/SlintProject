...
function Productos({ handleProd,setCant,productos, conf, prodFoc,isProd}) {
    const [unfoc,setUnfoc]=useState(prodFoc?"":"not-focused");
    const [prods, setProds] = useState(productos.length > 0 ? productos.map((prod, i) => {
        return <Producto setCant={setCant} handleProd={handleProd} key={i} producto={prod} conf={conf} i={i} />
    }) : "")
    useEffect(()=>{
