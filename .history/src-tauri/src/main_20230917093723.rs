...
const PATH: &str = "src/Productos.json";
...
fn crear_file<'a>(escritura: &impl Serialize) {
    ...
    match file {
        Ok(mut a) => {
            if let Err(e) = writeln!(a, "{}", serde_json::to_string_pretty(escritura)) {
...
#[tauri::command]
fn buscador(_name: &str) -> String {
    let pr: Producto = Producto {
        tipo_producto: "Harina".to_string(),
        marca: String::from("Pureza"),
        variedad: String::from("0000"),
        cantidad: Presentacion::Grs(1000.0),
    };
    format!(
