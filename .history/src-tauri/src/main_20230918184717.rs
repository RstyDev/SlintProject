...
impl Sistema{
    ...
    cantidad: &str,
    presentacion: &str){
        self.productos.push(Producto { codigo_de_barras:codigo_de_barras.parse().unwrap(), precio_de_venta: precio_de_venta.parse().unwrap(), porcentaje: porcentaje.parse().unwrap(), precio_de_costo: precio_de_costo.parse().unwrap(), tipo_producto: (), marca: (), variedad: (), cantidad: () })
