...
document.addEventListener('DOMContentLoaded', function changeSaleprice() {
  let percent;
  if (document.querySelector('#porcentaje').value = null) {
    percent = 40;
  } else {
    percent = document.querySelector('#porcentaje').value;
  }

  console.log(percent);
  let sale = document.querySelector('#precio_de_compra');
  console.log(sale);
  document.querySelector('#precio_de_venta').value = parseFloat(sale).value * parseFloat(percent = 40)
})
