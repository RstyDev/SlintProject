...
window.addEventListener("DOMContentLoaded", () => {
  ...
  venta.addEventListener('input', () => {
    let costo = document.querySelector('#precio_de_costo').value;
    console.log(document.querySelector('#precio_de_costo').value);
    if (!isNaN(document.querySelector('#precio_de_costo').value)) {
      let floatventa=parseFloat(venta.value);
      let floatcosto=parseFloat(costo);
      document.querySelector('#porcentaje').value =((parseFloat(floatventa)/parseFloat(floatcosto))*100)-100;
    }else{
