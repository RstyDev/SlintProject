...
impl<'a> Sistema {
    ...
    pub fn test(
        ...
        Runtime::new()
            .unwrap()
            .block_on(async { fresh(w1.as_ref()).await });
        let configs = Runtime::new()
            .unwrap()
            .block_on(async { Config::get_or_def(&write_db.as_ref()).await })
            .unwrap();
        let caja = Runtime::new()
            .unwrap()
            .block_on(async { Caja::new(&write_db.as_ref(), Some(0.0), &configs).await })?;
        ...
        let sis = Sistema {
            ...
            ventas: Ventas {
                a: Runtime::new()
                    .unwrap()
                    .block_on(async { Venta::get_or_new(None, w2.as_ref(), true).await })?,
                b: Runtime::new()
                    .unwrap()
                    .block_on(async { Venta::get_or_new(None, w3.as_ref(), false).await })?,
            },
            proveedores: Vec::new(),
            relaciones: Vec::new(),
            stash: Vec::new(),
        ...
        Runtime::new()
            .unwrap()
            .block_on(async { Sistema::procesar_test(Arc::clone(&w2), r2).await })?;
        Ok(sis)
    }
    pub fn new(read_db: Arc<Pool<Sqlite>>, write_db: Arc<Pool<Sqlite>>) -> Res<Sistema> {
        ...
        let configs = Runtime::new()
            .unwrap()
            .block_on(async { Config::get_or_def(db.as_ref()).await })?;
        let caja = Runtime::new()
            .unwrap()
            .block_on(async { Caja::new(aux.as_ref(), Some(0.0), &configs).await })?;
        let stash = Vec::new();
        ...
        let sis = Sistema {
            ...
            ventas: Ventas {
                a: Runtime::new()
                    .unwrap()
                    .block_on(async { Venta::get_or_new(None, w1.as_ref(), true).await })?,
                b: Runtime::new()
                    .unwrap()
                    .block_on(async { Venta::get_or_new(None, w1.as_ref(), false).await })?,
            },
    ...
    pub fn cerrar_caja(&mut self, monto_actual: f32) -> Res<()> {
        ...
        Runtime::new()
            .unwrap()
            .block_on(self.caja.set_n_save(db.as_ref(), monto_actual))?;
        self.generar_reporte_caja();
        self.caja = Runtime::new().unwrap().block_on(async {
            Caja::new(self.write_db.as_ref(), Some(monto_actual), &self.configs).await
        })?;
        Ok(())
    }
    pub fn eliminar_usuario(&self, user: User) -> Res<()> {
        Runtime::new()
            .unwrap()
            .block_on(async { Db::eliminar_usuario(user, self.read_db.as_ref()).await })?;
        Ok(())
    }
    ...
    async fn procesar(
        ...
        let read_db2 = Arc::clone(&read_db);
        let a: tokio::task::JoinHandle<std::result::Result<(), E>> = Runtime::new().unwrap().spawn(async move {
            let medios = [CUENTA, "Efectivo", "Crédito", "Débito"];
            for i in 0..medios.len() {
    ...
    pub fn pagar_deuda_especifica(&self, cliente: i64, venta: Venta) -> Res<Venta> {
        Runtime::new().unwrap().block_on(async {
            Cli::pagar_deuda_especifica(cliente, &self.write_db, venta, &self.user).await
        })
    }
    pub fn pagar_deuda_general(&self, cliente: i64, monto: f32) -> Res<f32> {
        Runtime::new()
            .unwrap()
            .block_on(async { Cli::pagar_deuda_general(cliente, &self.write_db, monto).await })
    }
    ...
    fn cerrar_venta(&mut self, pos: bool) -> Res<()> {
        Runtime::new()
            .unwrap()
            .block_on(async { self.venta(pos).guardar(pos, self.write_db()).await })?;
        ...
        self.set_venta(
            pos,
            Runtime::new().unwrap().block_on(async {
                Venta::get_or_new(Some(self.arc_user()), self.write_db(), pos).await
            })?,
        );
        Ok(())
    }
    pub fn hacer_ingreso(&self, monto: f32, descripcion: Option<Arc<str>>) -> Res<()> {
        ...
        Runtime::new()
            .unwrap()
            .block_on(async { self.caja.hacer_movimiento(mov, &self.write_db).await })
    }
    ...
    pub fn get_deuda(&self, cliente: Cli) -> Res<f32> {
        Runtime::new()
            .unwrap()
            .block_on(async { cliente.get_deuda(&self.read_db).await })
    }
    ...
    pub fn set_cliente(&mut self, id: i64, pos: bool) -> Res<()> {
        if pos {
            Runtime::new()
                .unwrap()
                .block_on(async { self.ventas.a.set_cliente(id, &self.read_db).await })
        } else {
            Runtime::new()
                .unwrap()
                .block_on(async { self.ventas.b.set_cliente(id, &self.read_db).await })
        }
    }
