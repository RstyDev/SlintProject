...
impl<'a> Sistema {
    ...
    pub fn test(
        ...
        Runtime::new().unwrap().block_on(async {Sistema::procesar_test(Arc::clone(&w2), r2).await})?;
        Ok(sis)
    }
    pub fn new(read_db: Arc<Pool<Sqlite>>, write_db: Arc<Pool<Sqlite>>) -> Res<Sistema> {
        ...
        let configs = Runtime::new().unwrap().block_on(async {Config::get_or_def(db.as_ref()).await})?;
        let caja = Runtime::new().unwrap().block_on(async {Caja::new(aux.as_ref(), Some(0.0), &configs).await})?;
        let stash = Vec::new();
        ...
        let sis = Sistema {
            ...
            configs,
            ventas: Ventas {
                a: Runtime::new().unwrap().block_on(async {Venta::get_or_new(None, w1.as_ref(), true).await})?,
                b: Runtime::new().unwrap().block_on(async {Venta::get_or_new(None, w1.as_ref(), false).await})?,
            },
        ...
        Runtime::new().unwrap().block_on(async {Sistema::procesar(
    ...
    pub fn cerrar_caja(&mut self, monto_actual: f32) -> Res<()> {
        ...
        self.caja = Runtime::new().unwrap().block_on(async {Caja::new(
    ...
    pub fn eliminar_usuario(&self, user: User) -> Res<()> {
        Runtime::new().unwrap().block_on(async {Db::eliminar_usuario(user, self.read_db.as_ref()).await })?;
        Ok(())
    }
    ...
    pub fn pagar_deuda_general(&self, cliente: i64, monto: f32) -> Res<f32> {
        Runtime::new().unwrap().block_on(async {Cli::pagar_deuda_general(cliente, &self.write_db, monto).await })
    }
    ...
    pub fn agregar_proveedor(&mut self, proveedor: &str, contacto: Option<i64>) -> Res<()> {
        Runtime::new().unwrap().block_on(async {Proveedor::new_to_db(proveedor, contacto, self.write_db()).await })?;
        Ok(())
    }
    ...
    fn cerrar_venta(&mut self, pos: bool) -> Res<()> {
        Runtime::new().unwrap().block_on(async {self.venta(pos).guardar(pos, self.write_db()).await })?;
        self.registro.push(self.venta(pos).clone());
        println!("{:#?}", self.venta(pos));
        Runtime::new().unwrap().block_on( async{
            self.update_total(self.venta(pos).monto_total(), &self.venta(pos).pagos()).await }
        )?;
        self.set_venta(
            pos,
            Runtime::new().unwrap().block_on(async {Venta::get_or_new(
                Some(self.arc_user()),
                self.write_db(),
                pos,
            ).await })?,
    ...
    pub fn hacer_ingreso(&self, monto: f32, descripcion: Option<Arc<str>>) -> Res<()> {
        ...
        Runtime::new().unwrap().block_on(async {self.caja.hacer_movimiento(mov, &self.write_db).await })
    }
    ...
    pub fn get_deuda(&self, cliente: Cli) -> Res<f32> {
        Runtime::new().unwrap().block_on(async {cliente.get_deuda(&self.read_db).await })
    }
    pub fn get_deuda_detalle(&self, cliente: Cli) -> Res<Vec<Venta>> {
        Runtime::new().unwrap().block_on(async {cliente.get_deuda_detalle(&self.read_db, self.user()).await })
    }
    pub fn eliminar_valuable(&self, val: V) {
        Runtime::new().unwrap().block_on(async {val.eliminar(self.write_db.as_ref()).await });
    }
    pub fn editar_valuable(&self, val: V) {
        Runtime::new().unwrap().block_on(async {val.editar(self.write_db.as_ref()).await });
    }
    ...
    pub fn set_cliente(&mut self, id: i64, pos: bool) -> Res<()> {
        if pos {
            Runtime::new().unwrap().block_on(async {self.ventas.a.set_cliente(id, &self.read_db).await })
        } else {
            Runtime::new().unwrap().block_on(async {self.ventas.b.set_cliente(id, &self.read_db).await })
        }
    }
