...
mod tests {
    ...
    fn agregar_proveedor_sin_contacto_test() {
        ...
        let provs =
            Runtime::new().unwrap().block_on(async app.state::<Mutex<Sistema>>().lock().unwrap().proveedores())
                .clone();
        assert!(
    ...
    fn agregar_proveedor_con_contacto_test() {
        ...
        let provs =
            Runtime::new().unwrap().block_on(app.state::<Mutex<Sistema>>().lock().unwrap().proveedores())
                .clone();
        assert!(
            provs.len() == 1
