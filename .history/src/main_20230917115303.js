...
window.addEventListener("DOMContentLoaded", () => {
  var element = document.querySelector('#precio_de_compra').addEventListener('input', () => {
    let percent = document.querySelector('#porcentaje').value;
    console.log(percent);
    let sale = document.querySelector('#precio_de_compra').value;
    console.log(sale);
    document.querySelector('#precio_de_venta').value = parseFloat(sale) * parseFloat(percent)
  });
});
