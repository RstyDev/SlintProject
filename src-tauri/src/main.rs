...
use mods::{
    cmd::*,
     Caja, Cli, Config, Pago, Rango, Result as Res, Sistema, User, Valuable as V, Venta,
};
#[tauri::command]
fn agregar_cliente(
...
fn main() {
    let write_db = db(true);
    let read_db = db(false);
    block_on(fresh(&write_db));
    block_on(async {
    ...
    let app = tauri::Builder::default()
        .manage(Mutex::new(Sistema::new(Arc::from(read_db),Arc::from(write_db)).unwrap()))
