...
impl Default for Vendedor{
    fn default() -> Self {
        Vendedor { id: 0, nombre: Arc::from("Vendedor 1") }
    }
}
...
impl Vendedor{
    ...
    pub async fn get_or_def(db:Arc<DatabaseConnection>)->Result<Vendedor,AppError>{
        let vend=entity::vendedor::Entity::find().order_by_desc(entity::vendedor::Column::Id).one(db.as_ref()).await?;
        let vendedor=match vend{
            Some(a) => Vendedor{ id: a.id, nombre: Arc::from(a.nombre) },
            None => {
                let vend= Vendedor { id: 0, nombre: Arc::from("Vendedor 1") };
