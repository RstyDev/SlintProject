...
impl User {
    pub async fn new_to_db(id: Arc<str>, nombre:Arc<str>,pass:i64,rango: &str, db:&DatabaseConnection)->Res<User>{
        let rango2=match rango{
            "Admin"=>Rango::Admin,
            "Cajero"=>Rango::Cajero,
            _=>panic!("No existe")
        };
        match entity::user::Entity::find().filter(entity::user::Column::UserId.eq(id.as_ref())).one(db).await?{
            Some(_) => Ok(User{ id, nombre, pass, rango: rango2 }),
            None => {
    ...
    pub fn new(id: Arc<str>,nombre:Arc<str>, pass: i64, rango: &str) -> User {
        ...
        User { id, nombre: nombre, pass, rango  }
    }
